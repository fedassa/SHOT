...
public:
	MeshPointsIndex(vtkPolyData* polyData);
	~MeshPointsIndex();
...
public:				
	...
	virtual void describe(vtkPolyData* cloud, Feature3D* & feat, double** & desc, int nPoints);								
...
private:	
	...
	void computeDesc (SHOTParams params, vtkPolyData *cloud, Feature3D *feat, double **desc, int startIndex, int endIndex);

	void interpolateDoubleChannel(vtkPolyData* cloud, vtkIdList* nnPoints, const std::vector<double> & distances, const double centralPoint[3], float rf[9], std::vector<double> & binDistanceShape, std::vector<double> & binDistanceColor, double* shot );

	void interpolateSingleChannel(vtkPolyData* cloud, vtkIdList* NNpoints, const std::vector<double> & distances, const double centralPoint[3], float rf[9], std::vector<double> & binDistance, int nr_bins, double* shot );
